...
# ---- Editorial palette -----------------------------------------------------
BRAND = "#6B4A8A"      # editorial muted violet
BRAND2 = "#7E5BA0"     # accent violet
INK = "#1A1A17"
MUTED = "#5B564B"
BODY = "#5B564B"
ACCENT = BRAND
...
st.markdown(
    ...
    .persona-card {{ background:#fff; border-radius:14px; padding:16px 20px; margin:8px 0;
        border:1px solid {LINE}; border-left:3px solid var(--persona-color, {BRAND});
        box-shadow:0 1px 2px rgba(26,26,23,.03); }}
    .persona-card h4 {{ font-family:'{SERIF}', serif; margin:0 0 6px 0;
        font-size:18px; font-weight:500; color:{INK}; letter-spacing:-.2px; }}
    .persona-card .meta {{ font-size:13px; color:{BODY}; margin-bottom:10px; }}
    .persona-card .action {{ font-size:14px; color:{INK}; line-height:1.55; }}
...
def note(text: str, tone: str = "neutral") -> None:
    bg = {"brand": "#EFE7F0", "good": "#E8EFE8", "warn": "#F3EBD8", "neutral": SOFT}[tone]
